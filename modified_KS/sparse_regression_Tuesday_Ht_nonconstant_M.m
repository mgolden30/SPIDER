...
Hts = [0.2, 0.4, 0.8, 1.6, 3.2, 6.4]; %dimensionless
t_per_subdomain = 0.8; %hold this constant
x_per_subdomain = 1; %hold this constant

res_4  = zeros(numel(Hts),1);
res_as = zeros(numel(Hts),1);
...
%  M = 4096;
%load training data
load("seed_1_Ht2_" + Ht + ".mat")
test = load("seed_543212345_Ht2_" + Ht + ".mat");
...
T = 20;
M = 2048;
nt = round(t_per_subdomain/T*M*Hts);

scatter( Hts, res_4, 100, 'filled' );
title("four term residual");
xlabel("$H_t (\textrm{correlation time})$", 'Interpreter', 'latex' );
...
saveas(gcf, 'figs/4_term_and_full_Ht2.png')
