function [valid, fields, derivs, digits] = check_library_term( i, num_fields, dim, max_symbol_length )
  %{
  PURPOSE:
  ...
  for i = 1:num_fields
    ...
    %For this case, there is only one field. No reason to subsample
    der = derivs(I); %subsample to size [dim, sum(I)];
    %der = sum( der.*( max_symbol_length.^(0:dim-1)') ); %map to a [1, sum(I)];
